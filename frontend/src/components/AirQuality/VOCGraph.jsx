...
export default function VOCGraph() {
  const plotRef = useRef(null);
  const plotInstance = useRef(null);
  const [data, setData] = useState([[], []]); // [timestamps, vocValues]
  ...
  useEffect(() => {
    ...
    const initialData = [
      ...
      Array.from({ length: 10 }, () => 200 + Math.random() * 50),
    ];
    setData(initialData);
    ...
    const opts = {
      ...
      series: [
        {},
        { label: "VOC", stroke: "#facc15", width: 2 }, // yellow
      ],
      axes: [
        {
          stroke: "white",
        ...
        { stroke: "white", font: "12px sans-serif", label: "ppm" },
      ],
    };
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const now = Math.floor(Date.now() / 1000);
      const newValue = 200 + Math.random() * 50;
      setData((prev) => {
