...
export default function HumidityGraph() {
  ...
  useEffect(() => {
    ...
    const initialData = [
      ...
      Array.from({ length: 10 }, () => 40 + Math.random() * 20),
    ];
    setData(initialData);
    ...
    const opts = {
      ...
      series: [
        {},
        { label: "Humidity", stroke: "#60a5fa", width: 2 }, // blue
      ],
      axes: [
        {
          stroke: "white",
        ...
        { stroke: "white", font: "12px sans-serif", label: "%RH" },
      ],
    };
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const now = Math.floor(Date.now() / 1000);
      const newValue = 40 + Math.random() * 20;
      setData((prev) => {
